...
BaseBot::BaseBot(const std::string &arg_Token,
                 const int64_t &Master_Id,
...
void BaseBot::setServerApiUrl(const std::string &Server_Api_Url) {
    ...
    this->sending_message_url_base =
        fmt::format("{}/bot{}/sendMessage?chat_id={}&text=", this->server_api_url, token, master_id);
    this->sending_document_url = fmt::format("{}/bot{}/sendDocument", this->server_api_url, token);
    this->sending_photo_url = fmt::format("{}/bot{}/sendPhoto", this->server_api_url, token);
    this->sending_audio_url = fmt::format("{}/bot{}/sendAudio", this->server_api_url, token);
    this->sending_video_url = fmt::format("{}/bot{}/sendVideo", this->server_api_url, token);
    this->sending_voice_url = fmt::format("{}/bot{}/sendVoice", this->server_api_url, token);
    this->getting_file_url = fmt::format("{}/bot{}/getFile?file_id=", this->server_api_url, token);

    this->downloading_file_url = fmt::format("{}/file/bot{}", this->server_api_url, token);
}
...
BotResponse BaseBot::sendMessage(const std::string &Text_Message) {
    try {
        char *escaped =
            curl_easy_escape(nullptr, Text_Message.c_str(), static_cast<int>(Text_Message.size()));
        if(!escaped)
...
BotResponse
BaseBot::sendFile(const std::filesystem::path &File_Path, const std::string &arg_Caption) {
    try {
...
BotResponse
BaseBot::sendPhoto(const std::filesystem::path &Photo_Path, const std::string &arg_Caption) {
    try {
        rat::networking::MimeContext ctx;
...
BotResponse
BaseBot::sendAudio(const std::filesystem::path &Audio_Path, const std::string &arg_Caption) {
    try {
        rat::networking::MimeContext ctx;
...
BotResponse
BaseBot::sendVideo(const std::filesystem::path &Video_Path, const std::string &arg_Caption) {
    try {
        rat::networking::MimeContext ctx;
...
BotResponse
BaseBot::sendVoice(const std::filesystem::path &Voice_Path, const std::string &arg_Caption) {
    try {
        rat::networking::MimeContext ctx;
...
bool BaseBot::downloadFile(const std::string &File_Id, const std::filesystem::path &Out_Path) {
    ...
    const auto response = this->curl_client.sendHttpRequest(get_file_url.c_str(), http_buffer);
    ...
    try {
        ...
        const std::string file_url = fmt::format("{}/{}", this->downloading_file_url, file_path);

        const auto download_response = this->curl_client.download(file_url, Out_Path);
