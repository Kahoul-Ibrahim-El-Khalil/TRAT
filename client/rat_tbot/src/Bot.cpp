...
Bot::Bot(const std::string &arg_Token,
    ...
    this->getting_update_url = fmt::format("{}/bot{}/getUpdates?timeout={}&limit=1",
                                           this->server_api_url,
                                           arg_Token,
...
Update Bot::parseJsonToUpdate(std::vector<char> &&arg_Buffer) {
    ...
    try {
        ...
        auto doc = simdjson_parser.iterate(arg_Buffer.data(), json_size, buffer_size);
        ...
        for(auto result_item : results.value()) {
            ...
            if(auto res = update_obj.value()["update_id"].get_int64();
