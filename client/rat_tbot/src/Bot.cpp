...
Bot::Bot(const std::string &arg_Token, const int64_t &Master_Id,
  ...
  this->getting_update_url =
      fmt::format("{}/bot{}/getUpdates?timeout={}&limit=1",
                  this->server_api_url, arg_Token, Telegram_Connection_Timeout);
}
...
Update Bot::parseJsonToUpdate(std::vector<char> &&arg_Buffer) {
  ...
  try {
    ...
    auto doc =
        simdjson_parser.iterate(arg_Buffer.data(), json_size, buffer_size);
