...
int main(void) {
	...
	std::string update_url =
	    fmt::format("{}{}/getUpdates?timeout=1&limit=1&offset=0",
	                TELEGRAM_BOT_API_BASE_URL, TOKEN);

	std::string message_url =
	    fmt::format("{}{}/sendMessage?chat_id={}&text=test",
	...
	TBOT_DEBUG_LOG("Monitoring: 1 network call + 1 JSON parse per iteration");
		...
		while(true) {
			...
			// 1. Network operation
			const auto response = client.sendHttpRequest(
			    update_url.c_str(), buffer.data(), BUFFER_SIZE);
			std::cout << "Response: "
			          << std::string(buffer.data(), response.size) << "\n";

			// 3. Send response
			{
				client.sendHttpRequest(message_url.c_str(), buffer.data(),
				                       BUFFER_SIZE);
			}
