...
std::string readFile(const std::filesystem::path &File_Path) {
    try {
        if(!std::filesystem::exists(File_Path) || !isFile(File_Path))
            return {};
...
bool echo(const std::vector<std::string> &arg_Buffers, const std::filesystem::path &File_Path) {
    try {
        ...
        if(!ofs.is_open())
            return false;
        for(const std::string &buffer : arg_Buffers)
            ofs << buffer << '\n';
        return true;
    } catch(...) {
        return false;
...
size_t getFileSize(const std::filesystem::path &File_Path) {
    try {
        if(!rat::system::exists(File_Path) || !isFile(File_Path))
            return 0;
...
bool copyFile(const std::filesystem::path &Origin_Path, const std::filesystem::path &Target_Path) {
    try {
        if(!std::filesystem::exists(Origin_Path) || !isFile(Origin_Path))
            return false;
        std::filesystem::copy_file(Origin_Path,
                                   Target_Path,
...
bool renameFile(const std::filesystem::path &Original_Path,
    ...
    try {
        if(!std::filesystem::exists(Original_Path) || !isFile(Original_Path))
            return false;
