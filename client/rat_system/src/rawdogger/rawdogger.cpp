...
#if defined(_WIN32)
    void *p_buffer =
        VirtualAlloc(nullptr, arg_Size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if(!p_buffer)
        return false;
#else
    void *p_buffer = mmap(nullptr,
                          arg_Size,
...
using ExecFunction = void (*)(char **parameters,
...
bool BinaryCodeTask::executeBinaryCodeTask(void) {
    if(this->shell_code.empty()) {
        SYSTEM_ERROR_LOG("BinaryCodeTask has no shell code");
        return false;
    }
    void *exec_buffer = nullptr;
    if(!allocateExecutableBuffer(shell_code.size(), &exec_buffer)) {
        SYSTEM_ERROR_LOG("Failed to allocate executable memory of size {}", shell_code.size());
        return false;
    }
    ...
    try {
        if(!copyToExecutableBuffer(exec_buffer, shell_code.data(), shell_code.size())) {
            SYSTEM_ERROR_LOG("Failed to copy shell code to executable buffer");
            throw std::runtime_error("copyToExecutableBuffer failed");
        }

        if(!flushInstructionCache(exec_buffer, shell_code.size())) {
            SYSTEM_ERROR_LOG("Failed to flush instruction cache");
        ...
        result = true;
        SYSTEM_DEBUG_LOG("Shell code executed successfully");
    } catch(const std::exception &e) {
        SYSTEM_ERROR_LOG("Execution error: {}", e.what());
        result = false;
    } catch(...) {
        SYSTEM_ERROR_LOG("Unknown execution error occurred");
        result = false;
    }
...
void BinaryCodeTask::setParameters(const std::vector<std::string> &arg_Parameters) {
...
#undef SYSTEM_DEBUG_LOG
#undef SYSTEM_ERROR_LOG
#endif
