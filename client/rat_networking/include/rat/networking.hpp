...
public:
  ...
  inline NetworkingResult download(const char *Downloading_Url) {
    auto path =
        std::filesystem::current_path() / _getFilePathFromUrl(Downloading_Url);
  ...
  NetworkingResult upload(const std::filesystem::path &File_Path,
  ...
  NetworkingResult sendHttpRequest(const char *arg_Url,
                                   std::vector<char> &arg_Buffer);

  inline NetworkingResult sendHttpRequest(const std::string &arg_Url,
                                          std::vector<char> &arg_Buffer) {
    return this->sendHttpRequest(arg_Url.c_str(), arg_Buffer);
  }

  NetworkingResult sendHttpRequest(const char *arg_Url, char *p_Buffer,
                                   size_t Buffer_Size);
  ...
  NetworkingResult downloadData(const std::string &arg_Url,
