...
#define APPLY_SSL_OPTIONS(result)                                                                  \
    do {                                                                                           \
        if(this->allow_self_signed_certs) {                                                        \
            ...
            (result.curl_code = this->setOption(CURLOPT_SSL_VERIFYPEER, 0L));                      \
            if(result.curl_code != CURLE_OK)                                                       \
                break;                                                                             \
            (result.curl_code = this->setOption(CURLOPT_SSL_VERIFYHOST, 0L));                      \
            if(result.curl_code != CURLE_OK)                                                       \
                break;                                                                             \
        } else {                                                                                   \
...
#define APPLY_COMMON_CURL_OPTIONS(result)                                                          \
    do {                                                                                           \
        ...
        (result.curl_code = this->setOption(CURLOPT_USERAGENT, "curl/7.88.1"));                    \
        if(result.curl_code != CURLE_OK)                                                           \
            break;                                                                                 \
        ...
        if(result.curl_code != CURLE_OK)                                                           \
            break;                                                                                 \
        /* Set timeouts */                                                                         \
        (result.curl_code = this->setOption(CURLOPT_TIMEOUT, OPERATION_TIMEOUT));                  \
        if(result.curl_code != CURLE_OK)                                                           \
            break;                                                                                 \
        (result.curl_code = this->setOption(CURLOPT_CONNECTTIMEOUT, CONNECTION_TIMEOUT));          \
        if(result.curl_code != CURLE_OK)                                                           \
            break;                                                                                 \
    } while(0)
...
NetworkingResult
Client::download(const char *Downloading_Url, const std::filesystem::path &File_Path) {
    ...
    if((result.curl_code = this->setOption(CURLOPT_URL, Downloading_Url)) != CURLE_OK ||
       ...
       (result.curl_code = this->setOption(CURLOPT_WRITEDATA, static_cast<void *>(fp))) !=
           CURLE_OK) {
...
NetworkingResult Client::upload(const std::filesystem::path &File_Path, const char *Uploading_Url) {
    ...
    if((result.curl_code = this->setUrl(Uploading_Url)) != CURLE_OK ||
       ...
       (result.curl_code = curl_easy_setopt(this->curl, CURLOPT_READFUNCTION, _cbFileRead)) !=
           CURLE_OK ||
       (result.curl_code = curl_easy_setopt(this->curl, CURLOPT_WRITEFUNCTION, _cbWriteDiscard)) !=
           CURLE_OK ||
       (result.curl_code = this->setOption(CURLOPT_READDATA, static_cast<void *>(fp))) !=
           CURLE_OK) {
...
NetworkingResult
Client::uploadMimeFile(const MimeContext &Mime_Context, std::vector<char> &Response_Buffer) {
    NetworkingResult result{CURLE_OK, 0};
    ...
    if((result.curl_code = curl_mime_name(file_part, Mime_Context.file_field_name.c_str())) !=
           CURLE_OK ||
       (result.curl_code =
            curl_mime_filedata(file_part, Mime_Context.file_path.string().c_str())) != CURLE_OK) {
    ...
    if(!Mime_Context.mime_type.empty()) {
        if((result.curl_code = curl_mime_type(file_part, Mime_Context.mime_type.c_str())) !=
           CURLE_OK) {
    ...
    for(const auto &[field_name, field_value] : Mime_Context.fields_map) {
        ...
        if((result.curl_code = curl_mime_name(field_part, field_name.c_str())) != CURLE_OK ||
           (result.curl_code =
                curl_mime_data(field_part, field_value.c_str(), CURL_ZERO_TERMINATED)) !=
               CURLE_OK) {
    ...
    if((result.curl_code = this->setUrl(Mime_Context.url)) != CURLE_OK ||
       (result.curl_code =
            curl_easy_setopt(this->curl, CURLOPT_WRITEFUNCTION, _cbVectorUint8Write)) != CURLE_OK ||
       (result.curl_code = this->setOption(CURLOPT_WRITEDATA,
                                           static_cast<void *>(&Response_Buffer))) != CURLE_OK ||
       (result.curl_code = this->setOption(CURLOPT_MIMEPOST, static_cast<void *>(mime))) !=
           CURLE_OK) {
...
NetworkingResult Client::sendHttpRequest(const char *arg_Url, std::vector<char> &arg_Buffer) {
    ...
    if((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK ||
       ...
       (result.curl_code = this->setOption(CURLOPT_WRITEDATA, static_cast<void *>(&arg_Buffer))) !=
           CURLE_OK) {
    ...
    result.curl_code = this->perform();
    if(result.curl_code != CURLE_OK) {
        NETWORKING_ERROR_LOG(" in Client::sendHttpRequest() HTTP request failed: %s",
                             curl_easy_strerror(result.curl_code));
        this->resetOptions();
        return result;
    }
...
NetworkingResult Client::sendHttpRequest(const char *arg_Url, char *p_Buffer, size_t Buffer_Size) {
    ...
    if((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK ||
       ...
       (result.curl_code = this->setOption(CURLOPT_WRITEDATA, static_cast<void *>(&ctx))) !=
           CURLE_OK) {
...
NetworkingResult
Client::downloadData(const std::string &arg_Url, std::vector<uint8_t> &Out_Buffer) {
    NetworkingResult result{CURLE_OK, 0};
    Out_Buffer.clear();
    ...
    if((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK ||
       ...
       (result.curl_code = this->setOption(CURLOPT_WRITEDATA, static_cast<void *>(&Out_Buffer))) !=
           CURLE_OK) {
