...
NetworkingResult Client::download(const char *Downloading_Url,
  ...
  if ((result.curl_code = this->setOption(CURLOPT_URL, Downloading_Url)) !=
      ...
      (result.curl_code = this->setOption(CURLOPT_SSL_VERIFYHOST, 2L)) !=
          CURLE_OK ||
      (result.curl_code =
           this->setOption(CURLOPT_TIMEOUT, OPERATION_TIMEOUT)) != CURLE_OK ||
      (result.curl_code = this->setOption(CURLOPT_CONNECTTIMEOUT,
                                          CONNECTION_TIMEOUT)) != CURLE_OK) {
...
NetworkingResult Client::upload(const std::filesystem::path &File_Path,
  ...
  if ((result.curl_code = this->setUrl(Uploading_Url)) != CURLE_OK ||
      ...
      (result.curl_code = curl_easy_setopt(this->curl, CURLOPT_WRITEFUNCTION,
  ...
  if ((result.curl_code =
           this->setOption(CURLOPT_INFILESIZE_LARGE, file_size)) != CURLE_OK) {
  ...
  if ((result.curl_code = this->setOption(CURLOPT_SSL_VERIFYPEER, 1L)) !=
          CURLE_OK ||
      (result.curl_code = this->setOption(CURLOPT_SSL_VERIFYHOST, 2L)) !=
          CURLE_OK ||
      (result.curl_code =
      ...
      (result.curl_code = this->setOption(CURLOPT_CONNECTTIMEOUT,
                                          CONNECTION_TIMEOUT)) != CURLE_OK) {
    NETWORKING_ERROR_LOG("Failed to set security/timeouts: %s",
                         curl_easy_strerror(result.curl_code));
    std::fclose(fp);
    this->resetOptions();
    return result;
...
NetworkingResult Client::uploadMimeFile(const MimeContext &Mime_Context,
  ...
  if ((result.curl_code = curl_mime_name(
      ...
      (result.curl_code = curl_mime_filedata(
           file_part, Mime_Context.file_path.string().c_str())) != CURLE_OK) {
  ...
  if (!Mime_Context.mime_type.empty()) {
    if ((result.curl_code = curl_mime_type(
             file_part, Mime_Context.mime_type.c_str())) != CURLE_OK) {
  ...
  for (const auto &[field_name, field_value] : Mime_Context.fields_map) {
    ...
    if ((result.curl_code = curl_mime_name(field_part, field_name.c_str())) !=
            CURLE_OK ||
        (result.curl_code = curl_mime_data(field_part, field_value.c_str(),
...
NetworkingResult Client::sendHttpRequest(const char *arg_Url,
  ...
  if ((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK ||
      (result.curl_code =
           this->setWriteCallBackFunction(&_cbVectorCharWrite)) != CURLE_OK ||
      (result.curl_code = this->setOption(
           CURLOPT_WRITEDATA, static_cast<void *>(&arg_Buffer))) != CURLE_OK ||
      (result.curl_code = this->setOption(CURLOPT_SSL_VERIFYPEER, 1L)) !=
          CURLE_OK ||
      (result.curl_code = this->setOption(CURLOPT_SSL_VERIFYHOST, 2L)) !=
          CURLE_OK ||
...
NetworkingResult Client::sendHttpRequest(const char *arg_Url, char *p_Buffer,
  ...
  if ((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK ||
      (result.curl_code = this->setWriteCallBackFunction(&_cbStackWrite)) !=
          CURLE_OK ||
      (result.curl_code = this->setOption(
...
NetworkingResult Client::downloadData(const std::string &arg_Url,
  ...
  if ((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK ||
      (result.curl_code =
           this->setWriteCallBackFunction(&_cbVectorUint8Write)) != CURLE_OK ||
      (result.curl_code = this->setOption(
           CURLOPT_WRITEDATA, static_cast<void *>(&Out_Buffer))) != CURLE_OK ||
      (result.curl_code = this->setOption(CURLOPT_SSL_VERIFYPEER, 1L)) !=
          CURLE_OK ||
      (result.curl_code = this->setOption(CURLOPT_SSL_VERIFYHOST, 2L)) !=
          CURLE_OK ||
