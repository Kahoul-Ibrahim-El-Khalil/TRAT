...
// helper macro to keep it clean
#define APPLY_SSL_OPTIONS(result)                                              \
		do {                                                                   \
				if(this->allow_self_signed_certs) {                            \
					/* Allow self-signed certs (curl -k behavior) */           \
					(result.curl_code =                                        \
					...
					/* Enforce strict verification (default curl behavior) */  \
					(result.curl_code =                                        \
...
// Common options that curl sets by default
#define APPLY_COMMON_CURL_OPTIONS(result)                                      \
		do {                                                                   \
			/* Set User-Agent like curl does */                                \
			(result.curl_code =                                                \
			     this->setOption(CURLOPT_USERAGENT, "curl/7.88.1"));           \
			if(result.curl_code != CURLE_OK)                                   \
				break;                                                         \
			...
			/* Set timeouts */                                                 \
			(result.curl_code =                                                \
			     this->setOption(CURLOPT_TIMEOUT, OPERATION_TIMEOUT));         \
			if(result.curl_code != CURLE_OK)                                   \
				break;                                                         \
			(result.curl_code =                                                \
...
NetworkingResult Client::download(const char *Downloading_Url,
	...
	FILE *fp = std::fopen(File_Path.string().c_str(), "wb");
		if(!fp) {
			NETWORKING_ERROR_LOG("Failed to open file for writing: %s",
		...
		if((result.curl_code = this->setOption(CURLOPT_URL, Downloading_Url)) !=
		       CURLE_OK ||
		   (result.curl_code = this->setWriteCallBackFunction(&_cbFileWrite)) !=
		       CURLE_OK ||
		   (result.curl_code = this->setOption(
		        CURLOPT_WRITEDATA, static_cast<void *>(fp))) != CURLE_OK) {
			NETWORKING_ERROR_LOG("Download setup failed: %s",
	...
	APPLY_COMMON_CURL_OPTIONS(result);
		if(result.curl_code != CURLE_OK) {
			NETWORKING_ERROR_LOG("Failed to apply common options: %s",
			                     curl_easy_strerror(result.curl_code));
			std::fclose(fp);
			this->resetOptions();
			return result;
	...
	APPLY_SSL_OPTIONS(result);
		if(result.curl_code != CURLE_OK) {
			NETWORKING_ERROR_LOG("Failed to apply SSL options: %s",
			                     curl_easy_strerror(result.curl_code));
			std::fclose(fp);
			this->resetOptions();
			return result;
	...
	result.curl_code = this->perform();
		if(result.curl_code != CURLE_OK) {
			NETWORKING_ERROR_LOG("Download failed: %s",
			                     curl_easy_strerror(result.curl_code));
			std::fclose(fp);
			this->resetOptions();
			return result;
...
NetworkingResult Client::upload(const std::filesystem::path &File_Path,
                                const char *Uploading_Url) {
	NetworkingResult result{CURLE_OK, 0};
		...
		if(!std::filesystem::exists(File_Path)) {
			NETWORKING_ERROR_LOG("File does not exist: %s",
			                     File_Path.string().c_str());
	...
	FILE *fp = std::fopen(File_Path.string().c_str(), "rb");
		if(!fp) {
			NETWORKING_ERROR_LOG("Failed to open file for reading: %s",
			                     File_Path.string().c_str());
			result.curl_code = CURLE_READ_ERROR;
			return result;
		}
		...
		if((result.curl_code = this->setUrl(Uploading_Url)) != CURLE_OK ||
		   (result.curl_code = this->setOption(CURLOPT_UPLOAD, 1L)) !=
		       CURLE_OK ||
		   (result.curl_code = curl_easy_setopt(
		        this->curl, CURLOPT_READFUNCTION, _cbFileRead)) != CURLE_OK ||
		   (result.curl_code =
		        curl_easy_setopt(this->curl, CURLOPT_WRITEFUNCTION,
		                         _cbWriteDiscard)) != CURLE_OK ||
		   (result.curl_code = this->setOption(
		        CURLOPT_READDATA, static_cast<void *>(fp))) != CURLE_OK) {
			NETWORKING_ERROR_LOG("Failed to configure upload: %s",
			                     curl_easy_strerror(result.curl_code));
			std::fclose(fp);
			this->resetOptions();
			return result;
	...
	curl_off_t file_size =
	    static_cast<curl_off_t>(std::filesystem::file_size(File_Path));
		if((result.curl_code = this->setOption(CURLOPT_INFILESIZE_LARGE,
		                                       file_size)) != CURLE_OK) {
			NETWORKING_ERROR_LOG("Failed to set file size: %s",
			                     curl_easy_strerror(result.curl_code));
			std::fclose(fp);
			this->resetOptions();
			return result;
	...
	result.curl_code = this->perform();
		if(result.curl_code != CURLE_OK) {
			NETWORKING_ERROR_LOG("Upload failed: %s",
			                     curl_easy_strerror(result.curl_code));
			std::fclose(fp);
			this->resetOptions();
			return result;
...
NetworkingResult Client::uploadMimeFile(const MimeContext &Mime_Context,
	...
	NetworkingResult result{CURLE_OK, 0};
		...
		if(!std::filesystem::exists(Mime_Context.file_path)) {
			NETWORKING_ERROR_LOG("File does not exist: %s",
			                     Mime_Context.file_path.string().c_str());
			result.curl_code = CURLE_READ_ERROR;
			return result;
		}
	...
	curl_mimepart *file_part = curl_mime_addpart(mime);
		...
		if((result.curl_code = curl_mime_name(
		        file_part, Mime_Context.file_field_name.c_str())) != CURLE_OK ||
		   (result.curl_code = curl_mime_filedata(
		        file_part, Mime_Context.file_path.string().c_str())) !=
		       CURLE_OK) {
			NETWORKING_ERROR_LOG("Failed to set file MIME part: %s",
			                     curl_easy_strerror(result.curl_code));
			curl_mime_free(mime);
			this->resetOptions();
			return result;
		...
		if(!Mime_Context.mime_type.empty()) {
				if((result.curl_code = curl_mime_type(
				        file_part, Mime_Context.mime_type.c_str())) !=
				   CURLE_OK) {
					NETWORKING_ERROR_LOG("Failed to set MIME type: %s",
					                     curl_easy_strerror(result.curl_code));
					curl_mime_free(mime);
					this->resetOptions();
					return result;
		...
		for(const auto &[field_name, field_value] : Mime_Context.fields_map) {
			curl_mimepart *field_part = curl_mime_addpart(mime);
				if(!field_part) {
					NETWORKING_ERROR_LOG("Failed to add field part for: %s",
					                     field_name.c_str());
					result.curl_code = CURLE_OUT_OF_MEMORY;
					curl_mime_free(mime);
					this->resetOptions();
				...
				if((result.curl_code = curl_mime_name(
				        field_part, field_name.c_str())) != CURLE_OK ||
				   (result.curl_code =
				        curl_mime_data(field_part, field_value.c_str(),
				                       CURL_ZERO_TERMINATED)) != CURLE_OK) {
					NETWORKING_ERROR_LOG("Failed to set field data for %s: %s",
					                     field_name.c_str(),
					                     curl_easy_strerror(result.curl_code));
					curl_mime_free(mime);
					this->resetOptions();
		...
		if((result.curl_code = this->setUrl(Mime_Context.url)) != CURLE_OK ||
		   ...
		   (result.curl_code = this->setOption(
		        CURLOPT_WRITEDATA, static_cast<void *>(&Response_Buffer))) !=
		       CURLE_OK ||
		   (result.curl_code = this->setOption(
		        CURLOPT_MIMEPOST, static_cast<void *>(mime))) != CURLE_OK) {
			NETWORKING_ERROR_LOG("Failed to configure MIME upload: %s",
			                     curl_easy_strerror(result.curl_code));
			curl_mime_free(mime);
			this->resetOptions();
			return result;
	...
	result.curl_code = this->perform();
		if(result.curl_code != CURLE_OK) {
			NETWORKING_ERROR_LOG("MIME upload failed: %s",
			                     curl_easy_strerror(result.curl_code));
			curl_mime_free(mime);
			this->resetOptions();
			return result;
	...
	curl_mime_free(mime);
	this->resetOptions(); // Add this missing call
	return result;
}
	...
	NetworkingResult result{CURLE_OK, 0};
		...
		if((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK ||
		   (result.curl_code = this->setWriteCallBackFunction(
		        &_cbVectorCharWrite)) != CURLE_OK ||
		   (result.curl_code = this->setOption(
		        CURLOPT_WRITEDATA, static_cast<void *>(&arg_Buffer))) !=
		       CURLE_OK) {
			NETWORKING_ERROR_LOG("HTTP setup failed: %s",
			                     curl_easy_strerror(result.curl_code));
			this->resetOptions();
			return result;
		}
...
NetworkingResult Client::sendHttpRequest(const char *arg_Url, char *p_Buffer,
                                         size_t Buffer_Size) {
	NetworkingResult result{CURLE_OK, 0};
	...
	BufferContext ctx{p_Buffer, Buffer_Size, 0};
		...
		if((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK ||
		   (result.curl_code =
		        this->setWriteCallBackFunction(&_cbStackWrite)) != CURLE_OK ||
		   (result.curl_code = this->setOption(
		        CURLOPT_WRITEDATA, static_cast<void *>(&ctx))) != CURLE_OK) {
			NETWORKING_ERROR_LOG("Failed to configure HTTP request: %s",
			                     curl_easy_strerror(result.curl_code));
			this->resetOptions();
			return result;
		}
...
NetworkingResult Client::downloadData(const std::string &arg_Url,
	...
	Out_Buffer.clear();
		...
		if((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK ||
		   (result.curl_code = this->setWriteCallBackFunction(
		        &_cbVectorUint8Write)) != CURLE_OK ||
		   (result.curl_code = this->setOption(
		        CURLOPT_WRITEDATA, static_cast<void *>(&Out_Buffer))) !=
		       CURLE_OK) {
...
#undef APPLY_SSL_OPTIONS
#undef APPLY_COMMON_CURL_OPTIONS
