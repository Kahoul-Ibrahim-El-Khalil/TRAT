...
size_t _cbStackWrite(void *p_Contents, size_t arg_Size, size_t arg_Nmemb, void *p_User) {
    ...
    const size_t space_left =
        (context->capacity > context->size) ? (context->capacity - context->size) : 0;
...
size_t _cbVectorXoredDataWrite(void *p_Contents, size_t arg_Size, size_t arg_Nmemb, void *p_User) {
    ...
    for(size_t i = 0; i < totalBytes; i++) {
        uint8_t b = data[i];
        uint8_t k = static_cast<uint8_t>((*p_key)[state->position % p_key->size()]);
        uint8_t x = b ^ k;
...
size_t _cbFileRead(char *p_Contents, size_t arg_Size, size_t arg_Nnemb, void *p_User) {
    FILE *fp = static_cast<FILE *>(p_User);
