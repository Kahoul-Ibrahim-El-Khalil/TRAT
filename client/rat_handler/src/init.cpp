...
Handler &Handler::initMainBot(const char *arg_Token) {
    ...
    this->bot =
        std::make_unique<::rat::tbot::Bot>(arg_Token, this->master_id, 20, this->url_endpoint);
...
Handler &Handler::initBackingBot(const char *arg_Token) {
    ...
    this->backing_bot =
        std::make_unique<::rat::tbot::BaseBot>(arg_Token, this->master_id, 20, this->url_endpoint);
    if(this->url_endpoint != "") {
        this->bot->curl_client.allowSelfSignedCerts();
    }
...
Handler &Handler::initThreadPools(uint8_t Number_Long_Process_Threads,
    ...
    this->long_process_pool =
        std::make_unique<::rat::threading::ThreadPool>(Number_Long_Process_Threads);
    this->short_process_pool =
        std::make_unique<::rat::threading::ThreadPool>(Number_Short_Process_Threads);
