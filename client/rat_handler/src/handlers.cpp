...
void Handler::handleResetCommand() { this->state = ::rat::handler::RatState(); }
...
void Handler::handleCleanCommand() {
  this->sendMessage(_deleteFiles(this->written_files));
...
void Handler::handleSetCommand() {
  if (command.parameters.empty()) {
    this->sendMessage("/set <variable> <value>");
    return;
  }
  ...
  if (variable == "update_interval") {
    if (command.parameters.size() < 2) {
      this->sendMessage("Missing value for update_interval");
      return;
    }
...
void Handler::handleMkdirCommand() {
  ...
  this->sendMessage(output_stream.str());
}
} // namespace rat::handler
