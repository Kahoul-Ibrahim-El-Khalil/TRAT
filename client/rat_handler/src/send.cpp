...
void Handler::sendMessage(const std::string &Message_Text) {
    if(this->bot->sendMessage(Message_Text) == ::rat::tbot::BotResponse::SUCCESS) {
...
void Handler::sendBackingMessage(const std::string &Message_Text) {
    ...
    if(this->backing_bot->sendMessage(Message_Text) == ::rat::tbot::BotResponse::SUCCESS) {
        HANDLER_DEBUG_LOG("Succeeded at sending: {}", Message_Text);
...
// cache.
void Handler::sendFile(const std::filesystem::path &File_Path, const std::string &arg_Caption) {
    if(this->bot->sendFile(File_Path, arg_Caption) == ::rat::tbot::BotResponse::SUCCESS) {
