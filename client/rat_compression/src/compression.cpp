...
bool zlibCompress(CompressionContext &Compression_Context) {
    if(Compression_Context.is_compressed || Compression_Context.uncompressed_size == 0 ||
...
bool zlibDecompress(CompressionContext &Compression_Context) {
    if(!Compression_Context.is_compressed || Compression_Context.compressed_size == 0 ||
       !Compression_Context.data)
        return false;
...
bool zlibCompressVector(std::vector<uint8_t> &arg_Vector, int Compression_Level) {
    ...
    CompressionContext compression_context(arg_Vector.data(),
                                           arg_Vector.size(),
                                           0,
                                           false,
                                           Compression_Level);
...
bool zstdCompress(CompressionContext &Compression_Context, int compressionLevel) {
    ...
    size_t compressedSize = ZSTD_compress(Compression_Context.data,
...
bool zstdDecompress(CompressionContext &Compression_Context) {
    ...
    size_t decompressedSize = ZSTD_decompress(Compression_Context.data,
                                              Compression_Context.uncompressed_size,
                                              Compression_Context.data,
                                              Compression_Context.compressed_size);
