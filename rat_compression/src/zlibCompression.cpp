...
bool zlibCompress(CompressionContext &Compression_Context) {
	if (Compression_Context.is_compressed || Compression_Context.uncompressed_size == 0 || !Compression_Context.data)
		return false;

	uLongf dest_len = static_cast<uLongf>(Compression_Context.compressed_size);
	int ret = compress2(Compression_Context.data, // destination buffer (pre-allocated)
						&dest_len,				  // will contain compressed size
						Compression_Context.data, // source buffer
						static_cast<uLong>(Compression_Context.uncompressed_size), static_cast<int>(Compression_Context.compression_level));
...
bool zlibDecompress(CompressionContext &Compression_Context) {
	if (!Compression_Context.is_compressed || Compression_Context.compressed_size == 0 || !Compression_Context.data)
		return false;

	uLongf dest_len = static_cast<uLongf>(Compression_Context.uncompressed_size);
	int ret = uncompress(Compression_Context.data, // destination buffer (pre-allocated)
...
bool zlibCompressVector(std::vector<uint8_t> &arg_Vector, int Compression_Level) {
	...
	CompressionContext compression_context(arg_Vector.data(), arg_Vector.size(), 0, false, Compression_Level);
...
bool zlibDecompressVector(std::vector<uint8_t> &arg_Vector, size_t Uncompressed_Size) {
	...
	CompressionContext compression_context(arg_Vector.data(), Uncompressed_Size, compressed_size, true);
