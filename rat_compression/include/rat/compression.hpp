...
/*This type is not supposed to own or manage the memory block it points to, simply because it is by design type agnostic, it could be a vector, array,
...
struct CompressionContext {
	...
	CompressionContext(uint8_t *p_Data, size_t Uncompressed_Size, size_t Compressed_Size, bool Is_Compressed,
					   uint8_t level = DEFAULT_COMPRESSION_LEVEL)
		: data(p_Data), uncompressed_size(Uncompressed_Size), compressed_size(Compressed_Size), is_compressed(Is_Compressed),
