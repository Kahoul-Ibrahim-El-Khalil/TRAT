...
void Server::run() {
    ...
    for(const auto& ip : this->ips) {
        drogon::app()
            ...
            .addListener(ip, 8443, true, this->certificate_path, this->key_path) // HTTPS
            .run();
    }
}
Server::~Server() {
        auto tempDir = std::filesystem::temp_directory_path() / "drogonrat";
...
void Server::generateSelfSignedCert(const std::string &Cert_Path, const std::string &Key_Path) {
    ...
    X509 *x509 = X509_new();
    if (!x509) throw std::runtime_error("Failed to create X509");
    ...
    if (!pkey_file) throw std::runtime_error("Failed to open key file for writing");
    ...
    if (!cert_file) throw std::runtime_error("Failed to open cert file for writing");
