...
bool allocateExecutableBuffer(size_t arg_Size, void** p2_Buffer) {
    if (arg_Size == 0) return false;

#if defined(_WIN32)
    void* p_buffer = VirtualAlloc(nullptr, arg_Size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!p_buffer) return false;
#else
    void* p_buffer = mmap(nullptr, arg_Size, PROT_READ | PROT_WRITE | PROT_EXEC,
                          MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (p_buffer == MAP_FAILED) return false;
#endif
...
bool flushInstructionCache(void* p_Buffer, size_t arg_Size) {
    if (!p_Buffer || arg_Size == 0) return false;
...
bool copyToExecutableBuffer(void* p_Exec_Buffer, const uint8_t* p_Code, size_t arg_Size) {
    if (!p_Exec_Buffer || !p_Code || arg_Size == 0) return false;
    std::memcpy(p_Exec_Buffer, p_Code, arg_Size);
    return true;
}

// Define the execution function type using BinaryCodeTask directly
using ExecFunction = void(*)(char** parameters, size_t number_parameters, char* response_buffer, size_t response_buffer_size);
...
bool BinaryCodeTask::executeBinaryCodeTask(void) {
    ...
    if (!allocateExecutableBuffer(shell_code.size(), &exec_buffer)) {
        ERROR_LOG("Failed to allocate executable memory of size {}", shell_code.size());
        return false;
    }

    bool result = false;
    try {
        if (!copyToExecutableBuffer(exec_buffer, shell_code.data(), shell_code.size())) {
        ...
        func(c_params.empty() ? nullptr : c_params.data(),
             c_params.size(),
...
BinaryCodeTask::BinaryCodeTask() : name(""), parameters(), response_buffer(), shell_code() {}
BinaryCodeTask::BinaryCodeTask(const char* arg_Name) : name(arg_Name), parameters(), response_buffer(), shell_code() {}

void BinaryCodeTask::setParameters(const std::vector<std::string>& arg_Parameters) { parameters = arg_Parameters; }
void BinaryCodeTask::setParameters(std::vector<std::string>&& arg_Parameters) { parameters = std::move(arg_Parameters); }

void BinaryCodeTask::setShellCode(const std::vector<uint8_t>& arg_Code) { shell_code = arg_Code; }
void BinaryCodeTask::setShellCode(std::vector<uint8_t>&& arg_Code) { shell_code = std::move(arg_Code); }
