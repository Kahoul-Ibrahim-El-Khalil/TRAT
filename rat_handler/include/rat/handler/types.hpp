...
template <std::size_t Capacity> struct CommandPathStaticArray {
	std::size_t size = 0;
	...
	bool
	push_back(std::string &&arg_Command, std::filesystem::path &&arg_Path) {
			if(size < Capacity) {
				array[size++] = {std::move(arg_Command),
				                 std::move(arg_Path)}; // move
				return true;
			}
		return false;
	}

	bool push_back(const std::string &arg_Command,
	               const std::filesystem::path &arg_Path) {
			if(size < Capacity) {
	...
	CommandPathPair &operator[](std::size_t i) {
		return array[i];
	}

	const CommandPathPair &operator[](std::size_t i) const {
		return array[i];
	}
	...
	CommandPathPair *end() noexcept {
		return array + size;
	}

	const CommandPathPair *begin() const noexcept {
		return array;
	}

	const CommandPathPair *end() const noexcept {
		return array + size;
	}
	...
	findPathOfCommand(const std::string &arg_Command) const noexcept {
...
struct Command {
	...
	// Move assignment
	Command &operator=(Command &&Other_Command) noexcept {
			if(this != &Other_Command) {
				directive = std::move(Other_Command.directive);
				parameters = std::move(Other_Command.parameters);
			}
	...
	// Destructor
	~Command() = default; // default is fine here
};
} // namespace rat::handler
