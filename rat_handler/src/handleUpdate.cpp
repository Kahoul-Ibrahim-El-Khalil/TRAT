...
void Handler::_dynamicSleep() {
	...
	double computed =
	    base_sleep_ms *
	    std::pow(growth_factor, static_cast<double>(this->empty_updates_count));
...
void Handler::handleUpdates() {
					...
					if(this->telegram_update->message.text.empty() ||
					   (this->telegram_update->message.text[0] != '!' &&
						...
						this->bot->sendMessage(
						    "Empty Message, an integrated command starts with "
						    "/, a dynamic one with ! ");
						continue;
					}
	...
	std::this_thread::sleep_for(
	    std::chrono::milliseconds(this->sleep_timeout_ms));
}
...
void Handler::dispatchDynamicCommand() {
	...
	auto it = this->state.command_path_map.find(directive);
		if(it == this->state.command_path_map.end()) {
			this->bot->sendMessage(
			    fmt::format("Unknown command: '{}'", directive));
			return;
		}
	...
	std::string new_message;
		if(!rest.empty()) {
			new_message =
			    fmt::format("/process {} {}", real_path.string(), rest);
		}
		else {
...
void Handler::dispatchIntegratedCommand() {
	HANDLER_DEBUG_LOG("Dispatching commands based on the update {}",
	                  this->telegram_update->id);
	this->parseTelegramMessageToCommand();
	...
	this->bot->sendMessage(
	    fmt::format("Unknown command: {}", this->command.directive));
}
