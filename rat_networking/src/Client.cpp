...
NetworkingResult Client::upload(const std::filesystem::path &File_Path, const char *Uploading_Url) {
	...
	if ((result.curl_code = this->setUrl(Uploading_Url)) != CURLE_OK || (result.curl_code = this->setOption(CURLOPT_UPLOAD, 1L)) != CURLE_OK ||
...
NetworkingResult Client::sendHttpRequest(const char *arg_Url, char *p_Buffer, size_t Buffer_Size) {
	...
	if ((result.curl_code = this->setUrl(arg_Url)) != CURLE_OK || (result.curl_code = this->setWriteCallBackFunction(&_cbStackWrite)) != CURLE_OK ||
