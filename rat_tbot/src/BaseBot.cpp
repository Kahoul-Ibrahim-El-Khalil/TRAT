...
BaseBot::BaseBot(const std::string& arg_Token, int64_t Master_Id, uint8_t Telegram_Connection_Timeout)
    ...
    this->sending_message_url_base = fmt::format("{}{}/sendMessage?chat_id={}&text=",
    ...
    this->http_buffer.resize(HTTP_RESPONSE_BUFFER_SIZE + simdjson::SIMDJSON_PADDING, 0);
...
// ---------------------- Getters ----------------------
std::string BaseBot::getToken() const { return token; }
...
void BaseBot::setUpdateIterval(uint16_t Update_Interval) { update_interval = Update_Interval; }
std::string BaseBot::getBotFileUrl() const { return getting_file_url; }

// ---------------------- Offset ----------------------
void BaseBot::setOffset() {
    const std::string init_url =
        fmt::format("{}{}/getUpdates?offset=-1&limit=1", TELEGRAM_BOT_API_BASE_URL, token);
    ...
    std::string json_raw(this->http_buffer.begin(), this->http_buffer.begin() + response.size);
...
BotResponse BaseBot::sendMessage(const std::string& Text_Message) {
    try {
        ...
        auto ok_res = doc["ok"].get_bool();
        if (ok_res.error()) return BotResponse::UNKNOWN_ERROR;
...
BotResponse BaseBot::sendFile(const std::filesystem::path& File_Path, const std::string& arg_Caption) {
    try {
        ...
        if (!arg_Caption.empty()) ctx.fields_map["caption"] = arg_Caption;

        if (extension == ".txt") ctx.mime_type = "text/plain";
        else if (extension == ".mp3") ctx.mime_type = "audio/mpeg";
        else if (extension == ".ogg") ctx.mime_type = "audio/ogg";
        else if (extension == ".mp4") ctx.mime_type = "video/mp4";
        else if (extension == ".pdf") ctx.mime_type = "application/pdf";
        else ctx.mime_type = "application/octet-stream";


        const auto response = this->curl_client.uploadMimeFile(ctx, this->http_buffer);
        if (response.size > 0) {
            std::string json_raw(this->http_buffer.begin(), this->http_buffer.begin() + response.size);
            simdjson::padded_string padded_json(json_raw);
            simdjson::ondemand::parser parser;
            ...
            auto ok_res = doc["ok"].get_bool();
            if (!ok_res.error() && ok_res.value()) return BotResponse::SUCCESS;
        }
...
BotResponse BaseBot::sendPhoto(const std::filesystem::path& Photo_Path, const std::string& arg_Caption) {
    try {
        ...
        const auto response = curl_client.uploadMimeFile(ctx, this->http_buffer);
        if (response.size > 0) {
            std::string json_raw(this->http_buffer.begin(), this->http_buffer.begin() + response.size);
            simdjson::padded_string padded_json(json_raw);
            simdjson::ondemand::parser parser;
...
bool BaseBot::downloadFile(const std::string& File_Id, const std::filesystem::path& Out_Path) {
    ...
    try {
        ...
        auto ok_res = doc["ok"].get_bool();
        if (ok_res.error() || !ok_res.value()) return false;

        std::string_view file_path = doc["result"]["file_path"].get_string();
        const std::string file_url =
            fmt::format("{}/file/bot{}/{}", TELEGRAM_API_URL, token, file_path);
